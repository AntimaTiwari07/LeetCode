             ...
             if(sum==(i+2))ans+=((i+2)*(i+2));
               ...
               for(int j = 0;j<t.size()-2+1;j++){
                   ...
                   int rem = sum - sum_string;
                    ...
                    if(final_sum == (i+2)){ans+=((i+2)*(i+2));found = true;}
               ...
               vector<pair<int,int>>tp;
                for(int j = 0;j<v[i].size()-3+1;j++){
                 ...
                 int sum_string = (v[i][j]-'0')+(v[i][j+1]-'0') +(v[i][j+2]-'0');
                   int rem = sum-sum_string;
                    int final_sum = rem+stoi(s);
                    if(final_sum==(i+2)){ans+=((i+2)*(i+2));break;}
                    tp.push_back({stoi(s),sum_string});
                }
                 if(tp.size()==3){
                    if(db[0].first+tp[tp.size()-1].first==(i+2)){ans+=((i+2)*(i+2));}
                    if(db[db.size()-1].first+tp[0].first==(i+2)){ans+=((i+2)*(i+2));} 
                 }
                 else if(tp.size()==4){
                       ...
                       if(db[0].first+tp[2].first+diff1 == (i+2)){ans+=(i+2)*(i+2);}
                       else  if(db[0].first+tp[3].first+diff2 == (i+2)){ans+=(i+2)*(i+2);}
                        else  if(db[1].first+tp[3].first+diff3 == (i+2)){ans+=(i+2)*(i+2);}
                         else  if(db[3].first+tp[0].first+diff4 == (i+2)){ans+=(i+2)*(i+2);}
                          else  if(db[4].first+tp[0].first+diff5 == (i+2)){ans+=(i+2)*(i+2);}
                           else  if(db[4].first+tp[1].first+diff6 == (i+2)){ans+=(i+2)*(i+2);}
                           else if(tp[0].first+tp[tp.size()-1].first==(i+2)){ans+=(i+2)*(i+2);}
                 }
               }
        }
        }
